...
function Apply-PuLIDReproducibilityInstrumentation {
    ...
    $NewAntelope = @(
        ...
        "        antelope_key = os.path.join(antelope_dir, 'glintr100.onnx')"
        "        if not os.path.exists(antelope_key):"
        "            if os.getenv('EXP001_OFFLINE_ASSETS'):"
        "                raise RuntimeError(f'Missing materialized AntelopeV2 asset: {antelope_key}')"
        "            snapshot_download("
